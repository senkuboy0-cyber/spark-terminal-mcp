    ...
    try:
        if name == "execute_command":
            ...
            if os.path.isfile(fp) or os.path.islink(fp):
                os.remove(fp); return ok(f"Deleted: {fp}")
            if args.get("recursive"):
                shutil.rmtree(fp); return ok(f"Dir deleted: {fp}")
            try:
                os.rmdir(fp); return ok(f"Empty dir deleted: {fp}")
            except OSError:
                return ok("Not empty. Use recursive=true.")

        elif name == "move_file":
            src, dst = args["source_path"], args["destination_path"]
            if not os.path.exists(src): return ok(f"Not found: {src}")
            Path(dst).parent.mkdir(parents=True, exist_ok=True)
            shutil.move(src, dst); return ok(f"Moved: {src} → {dst}")
            ...
            if os.path.isdir(src):
                if not args.get("recursive"): return ok("Use recursive=true for dirs.")
            ...
            if not os.path.exists(dp): return ok(f"Not found: {dp}")
            rows = []
            for entry in sorted(os.scandir(dp), key=lambda e: (not e.is_dir(), e.name)):
                if not args.get("show_hidden") and entry.name.startswith("."): continue
            ...
            if not results: return ok("No results.")
            ...
            resp = requests.request(args.get("method", "GET"), args["url"],
                headers=args.get("headers") or {}, data=args.get("body"),
            ...
            if mgr not in cmds: return ok(f"Unknown manager: {mgr}")
            ...
            fn = args.get("filter_name", "").lower(); rows = []
            for p in psutil.process_iter(["pid","name","cpu_percent","memory_percent","status"]):
                try:
                    i = p.info
                    if fn and fn not in i["name"].lower(): continue
            ...
            fp = args["file_path"]
            if not os.path.exists(fp): return ok(f"Not found: {fp}")
            ...
            src = args["source_path"]; out = os.path.join(DOWNLOADS_DIR, args["output_filename"])
            if not os.path.exists(src): return ok(f"Not found: {src}")
            ...
            arc = args["archive_path"]; dst = args.get("destination", os.path.dirname(arc))
            if not os.path.exists(arc): return ok(f"Not found: {arc}")
            os.makedirs(dst, exist_ok=True)
            if arc.endswith(".zip"):
                with zipfile.ZipFile(arc) as zf: zf.extractall(dst); count = len(zf.namelist())
            else:
                with tarfile.open(arc, "r:*") as tf: tf.extractall(dst); count = len(tf.getnames())
            ...
            t0 = time.time()
            try:
                s = socket.socket(); s.settimeout(int(args.get("timeout",5)))
                r = s.connect_ex((host, port)); s.close()
                return ok(f"Port {port} on {host}: {'OPEN' if r==0 else 'CLOSED'} ({round((time.time()-t0)*1000,2)}ms)")
            ...
            if not os.path.exists(fp): return ok(f"Not in /app/downloads: {args['filename']}")
            ...
            process_buffer.clear(); process_status.clear(); process_read_index.clear()
            for item in Path(TEMP_DIR).glob("*"):
                try: item.unlink() if item.is_file() else shutil.rmtree(item)
                except Exception: pass
            return ok("Cleared.")
...
async def require_bearer(request: Request):
    ...
    if not auth.startswith("Bearer "):
        raise HTTPException(
            ...
            headers={"WWW-Authenticate": f'Bearer realm="{_base_url(request)}"'})
...
async def mcp_streamable(request: Request, _claims=Depends(require_bearer)):
    """
    Streamable HTTP transport — what Gemini Spark uses.
    Each request is a self-contained MCP exchange.
    """
...
# ── SSE endpoint (legacy: Claude Desktop, etc.) ───────────────
@app.get("/sse")
